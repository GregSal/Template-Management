...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
  ...
  <xsl:template match="Fields">
      ...
      <div class="details">
        ...
        <table border="0" cellpadding="1" cellspacing="1">
          <tbody class="infotable">
            ...
            <tr>
              <xsl:if test="DeadSpaceLength &gt;= 0">
                <td width="140">Dead space length [<xsl:value-of select="DeadSpaceLength/@Unit"/>]:</td>
                <td width="140"><xsl:value-of select="DeadSpaceLength"/></td>
              </xsl:if>
            </tr>
          </tbody>
        </table>
            ...
            <table border="0" cellpadding="1" cellspacing="1">
              <thead class="tableheader">
                <tr align="center" valign="middle">
                  ...
                  <td width="140">Strength [Gy mm^2/s]</td>
                </tr>
              </thead>
              <tbody class="tablebody">
    ...
    <xsl:if test="count(BrachySeedCollection) &gt;  0">
      <h3>Plan Seed Collection Details</h3>
    </xsl:if>

    <!-- Seed Collection Details -->
    <xsl:for-each select="BrachySeedCollection">
      <div class="details">
        <h3>
          Seed Collection: <xsl:value-of select="@ID"/>
        </h3>
        <table border="0" cellpadding="1" cellspacing="1">
          <tbody class="infotable">
            <tr>
              <td width="140">Radioactive Source Model:</td>
              <td width="140">
                <xsl:value-of select="RadioactiveSourceModelId"/>
              </td>
            </tr>
            <xsl:if test="count( SeedPositions/SeedPosition  ) = 0">
              <tr>
                <td width="140">Strength [Gy mm^2/s]:</td>
                <td width="140">
                  <xsl:value-of select="SeedPositions/DefaultStrength"/>
                </td>
              </tr>
              <tr>
                <td width="140">Time [s]:</td>
                <td width="140">
                  <xsl:value-of select="SeedPositions/DefaultTime"/>
                </td>
              </tr>
            </xsl:if>
          </tbody>
        ...
        <h3>Seed Positions</h3>
        <!-- Seed Positions -->
        <xsl:if test="count( SeedPositions/SeedPosition  ) &gt; 0">
          <xsl:for-each select="SeedPositions">
            <table border="0" cellpadding="1" cellspacing="1">
              <thead class="tableheader">
                <tr align="center" valign="middle">
                  <td width="140">
                    Location  [<xsl:value-of select="SeedPosition/Tip/@Unit"/>]:
                  </td>
                  <td width="140">Strength [Gy mm^2/s]</td>
                  <td width="140">Time [s]</td>
                </tr>
              ...
              <tbody class="tablebody">
                <xsl:for-each select="SeedPosition">
                  <xsl:variable name="tipX" select="Tip/@X"/>
                  <xsl:variable name="tipY" select="Tip/@Y"/>
                  <xsl:variable name="tipZ" select="Tip/@Z"/>
                  <xsl:variable name="tailX" select="Tail/@X"/>
                  <xsl:variable name="tailY" select="Tail/@Y"/>
                  <xsl:variable name="tailZ" select="Tail/@Z"/>

                  <tr align="center" valign="middle">
                    <td width="120">
                      [<xsl:value-of select="format-number(0.5*($tailX + $tipX), '0.00')"/>, <xsl:value-of select="format-number(0.5*($tailY + $tipY), '0.00')"/>, <xsl:value-of select="format-number(0.5*($tailZ + $tipZ), '0.00')"/>]
                    </td>
                    <td width="140">
                      <xsl:value-of select="CurrentStrength"/>
                    </td>
                    <td width="140">
        ...
        <xsl:if test="count( RefLines/RefLine  ) &gt; 0">
          <xsl:for-each select="RefLines">
            <h3>Seed Collection Reference Lines</h3>
            <table border="0" cellpadding="1" cellspacing="1">
              <thead class="tableheader">
                <tr align="center" valign="middle">
                  <td width="140">Id</td>
                  <td width="160">
                    Point [<xsl:value-of select="RefLine/RefLinePoint/@Unit"/>]
                  </td>
                </tr>
              </thead>
              <tbody class="tablebody">
        ...
        <xsl:if test="count( RefPoints/RefPoint  ) &gt; 0">
          <xsl:for-each select="RefPoints">
            <h3>Seed Collection Reference Points</h3>
            <table border="0" cellpadding="1" cellspacing="1">
              <thead class="tableheader">
                <tr align="center" valign="middle">
                  <td width="100">Id</td>
                  <td width="130">
                    Planned Total Dose [<xsl:value-of select="RefPoint/TotalDosePlanned/@Unit"/>]
                  </td>
                  <td width="130">
                    Session Dose Limit [<xsl:value-of select="RefPoint/SessionDoseLimit/@Unit"/>]
                  </td>
                  <td width="120">
                    Daily Dose Limit [<xsl:value-of select="RefPoint/DailyDoseLimit/@Unit"/>]
                  </td>
                  <td width="120">
                    Total Dose Limit [<xsl:value-of select="RefPoint/TotalDoseLimit/@Unit"/>]
                  </td>
                  <td width="120">
                    Location [<xsl:value-of select="RefPoint/Location/@Unit"/>]
                  </td>
                </tr>
              </thead>
              <tbody class="tablebody">
                <xsl:for-each select="RefPoint">
                  <tr align="center" valign="middle">
                    ...
                    <td width="130">
                      <xsl:value-of select="TotalDosePlanned"/>
                    </td>
                    <td width="130">
                      <xsl:value-of select="SessionDoseLimit"/>
                    </td>
                    <td width="120">
                      <xsl:value-of select="DailyDoseLimit"/>
                    </td>
                    <td width="120">
                      <xsl:value-of select="TotalDoseLimit"/>
                    </td>
                    <td width="120">
                      [<xsl:value-of select="Location/@X"/>, <xsl:value-of select="Location/@Y"/>, <xsl:value-of select="Location/@Z"/>]
                    </td>
                  </tr>
                </xsl:for-each>
              </tbody>
